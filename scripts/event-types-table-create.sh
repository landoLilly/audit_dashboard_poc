...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Creating DynamoDB EventTypes Table (Local)                ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}\n"
...
aws dynamodb create-table \
  ...
  --attribute-definitions \
    AttributeName=EventTypeId,AttributeType=N \
    AttributeName=EventTypeName,AttributeType=S \
    AttributeName=Category,AttributeType=S \
  --key-schema \
    AttributeName=EventTypeId,KeyType=HASH \
  --global-secondary-indexes \
     "[{
            \"IndexName\": \"GSI1-EventTypeName\",
                \"KeySchema\": [{\"AttributeName\":\"EventTypeName\",\"KeyType\":\"HASH\"}],
...
if [ $? -eq 0 ]; then
    ...
    echo -e "\n${GREEN}✓ Base Table: EventTypes${NC}"
    echo -e "${GREEN}✓ GSI1: GSI1-EventTypeName${NC}"
    echo -e "${GREEN}✓ GSI2: GSI2-Category${NC}"
